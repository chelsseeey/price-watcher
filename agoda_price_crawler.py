...
def build_proxy(region: str):
    """region: 'KR' or 'US' -> playwright proxy dict or None"""
    server = PROXY.get(region)
    if not server:
        return None
    cfg = {"server": server}  # 반드시 http://host:port 형태
    if PROXY_USER.get(region):
        cfg["username"] = PROXY_USER[region]
    if PROXY_PASS.get(region):
        cfg["password"] = PROXY_PASS[region]
    return cfg
...
def _walk_price(obj):
    ...
    try:
        if isinstance(obj, dict):
            # 우선순위 높은 키들
            for k in ["finalPrice", "price", "priceAmount", "amount", "totalPrice", "grandTotal", "value", "amountTotal"]:
...
async def new_context(pw, region: str, device: str):
    ...
    proxy_cfg = build_proxy(region)  # ← 프록시 구성
    ...
    ctx = await browser.new_context(
        ...
        ignore_https_errors=True,  # 프록시로 인한 SSL 경고 회피
    )
