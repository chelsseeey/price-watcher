...
def main():
    ...
    # 환경 조합별 최저가 빈도 히트맵
    try:
        heatmap_min_frequency(data, cfg.get('freq','1H'))
    except Exception as e:
        st.info(f'히트맵 생성 실패: {e}')
    ...
    with st.expander("원본 데이터 보기", expanded=False):
        ...
        st.dataframe(show[cols] if cols else show, use_container_width=True, height=10000)
...
def heatmap_min_frequency(df, freq):
    """
    환경 조합별(Region × Device) 최저가 히트맵
    - 시점별 '최저가 동률'이면 해당 환경 **모두 1점씩** 부여
    - 0건 조합도 표시, 횟수/비율/정규화 모드 및 라벨 지원
    - 화면 배치: 히트맵 ▶ 옵션(하단 expander)
    """
    import pandas as pd

    if df is None or df.empty or not set(["ts_min","env","price"]).issubset(df.columns):
    ...
    def split_env(e: str):
        e = str(e)
        return e.split("-", 1) if "-" in e else (e, "all")
    t[["region","device"]] = t["env"].apply(lambda e: pd.Series(split_env(e)))

    # 리샘플 후 시점별 평균가격 테이블
    wide = (
        t.pivot_table(index="ts_min", columns="env", values="price", aggfunc="mean")
          .resample(freq).mean()
    )
    if wide is None or wide.empty:
        st.info("리샘플 결과가 비어 있습니다."); return

    # ─────────────────────────────────────────────────────────
    # 1) 먼저 session_state에서 옵션 값 가져와서 히트맵 그리기
    # ─────────────────────────────────────────────────────────
    tol = float(st.session_state.get("hm_tol", 0.0))
    mode = st.session_state.get(
        "hm_mode",
        "Absolute Frequency"
    )
    show_labels = bool(st.session_state.get("hm_labels", True))

    # 동률 처리
    wide_min = wide.min(axis=1)
    is_tie = (wide.sub(wide_min, axis=0).abs() <= tol)  # True/False

    # 각 env가 최저가였던 시점 수(동률이면 여러 env가 +1)
    env_counts = is_tie.sum(axis=0).rename("count").rename_axis("env").reset_index()

    # env -> region/device 분리
    env_counts["region"], env_counts["device"] = zip(
        *env_counts["env"].astype(str).map(
            lambda e: (e.split("-", 1)+["all"])[:2] if "-" in e else (e, "all")
        )
    )
    env_counts = env_counts[["region","device","count"]]

    # 0 포함 전체 그리드
    regions = sorted(t["region"].dropna().unique().tolist())
    devices = sorted(t["device"].dropna().unique().tolist())
    full = pd.MultiIndex.from_product([regions, devices], names=["region","device"])
    counts = env_counts.set_index(["region","device"]).reindex(full, fill_value=0).reset_index()

    # 표시 모드 적용
    if mode == "Absolute Frequency":
        counts["value"] = counts["count"].astype(float)
        color_title = "최저가 횟수"
        fmt = lambda v,c: f"{int(c)}"
    elif mode == "Relative Frequency (Global %)":
        tot = int(counts["count"].sum()) or 1
        counts["value"] = counts["count"] / tot * 100
        color_title = "최저가 비율(%)"
        fmt = lambda v,c: f"{v:.0f}%"
    elif mode == "Region-normalized Proportion":
        g = counts.groupby("region")["count"].transform("sum").replace(0,1)
        counts["value"] = counts["count"] / g * 100
        color_title = "해당 Region 내 비율(%)"
        fmt = lambda v,c: f"{v:.0f}%"
    else:  # Device-normalized
        g = counts.groupby("device")["count"].transform("sum").replace(0,1)
        counts["value"] = counts["count"] / g * 100
        color_title = "해당 Device 내 비율(%)"
        fmt = lambda v,c: f"{v:.0f}%"

    st.subheader("Lowest Price by Environment Combination")
    max_val = float(counts["value"].max()) if not counts.empty else 1.0

    base_chart = (
        alt.Chart(counts)
          .mark_rect(cornerRadius=6)
          .encode(
              x=alt.X("device:N", title="Device", sort=devices, axis=alt.Axis(labelAngle=0)),
              y=alt.Y("region:N", title="Region", sort=regions),
              color=alt.Color("value:Q", title=color_title,
                              scale=alt.Scale(domain=[0, max_val], scheme="blues")),
              tooltip=[
                  alt.Tooltip("region:N", title="Region"),
                  alt.Tooltip("device:N", title="Device"),
                  alt.Tooltip("count:Q", title="최저가 횟수", format="d"),
                  alt.Tooltip("value:Q", title=color_title,
                              format=".1f" if "비율" in color_title else ".0f")
              ]
          )
    ...
    if show_labels:
        counts["label"] = counts.apply(lambda r: fmt(r["value"], r["count"]), axis=1)
        labels = (
            alt.Chart(counts)
              .mark_text(baseline="middle", fontSize=12)
              .encode(
                  x="device:N", y="region:N", text="label:N",
                  color=alt.condition(
                      alt.datum.value > (max_val*0.6),
                      alt.value("white"), alt.value("black")
                  )
              )
        )
        chart = base_chart + labels
    else:
        chart = base_chart

    # 🔷 히트맵 먼저 출력
    st.altair_chart(
        chart.configure_axis(grid=False).configure_view(strokeOpacity=0),
        use_container_width=True
    )

    # ─────────────────────────────────────────────────────────
    # 2) 히트맵 아래쪽에 옵션 위젯 배치(다음 런에서 반영)
    # ─────────────────────────────────────────────────────────
    with st.expander("Options", expanded=True):
        st.caption("동률 처리: 동률 시 모두 집계")
        st.number_input("동률 허용 오차", value=tol, step=0.01,
                        help="허용 오차 범위 이내는 동일로 간주", key="hm_tol")
        st.radio(
            "Color Scale",
            ["Absolute Frequency", "Relative Frequency (Global %)",
             "Region-normalized Proportion", "Device-normalized Proportion"],
            horizontal=True, key="hm_mode"
        )
        st.checkbox("Show Values", value=show_labels, key="hm_labels")
